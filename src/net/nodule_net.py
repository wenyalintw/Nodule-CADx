...
class NoduleNet(nn.Module):
    ...
    def forward(self, inputs, truth_boxes, truth_labels, truth_masks, masks, split_combiner=None, nzhw=None):
        # features, feat_4 = data_parallel(self.feature_net, (inputs), output_device=-1, device_ids=-1); #print('fs[-1] ', fs[-1].shape)
    ...
    def forward2(self, inputs, bboxes):
        ...
        features = self.feature_net(inputs); #print('fs[-1] ', fs[-1].shape)
        fs = features[-1]
...
# 這行uncomment看看
        self.mask_targets = make_mask_target(self.cfg, self.mode, inputs, self.crop_boxes, truth_boxes, truth_labels, masks)
